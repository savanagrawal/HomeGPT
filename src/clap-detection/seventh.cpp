...
double find_per_data(const std::vector<double>& within_data, const std::string& find_file) {
    std::vector<double> y_find = load_audio_file(find_file);

    std::vector<double> c = convolve(within_data, y_find);
    
    double thres = *std::max_element(c.begin(), c.end()) / 2.0;
    double count = percentage_above_threshold(c, thres);
    std::fstream file;
...
const int WINDOW_SIZE = 44100; // 1 second of audio data

void analyze_data(const std::deque<float>& audioData) {
    std::vector<double> audioDataDouble(audioData.begin(), audioData.end());
    double per = find_per_data(audioDataDouble, "sample.wav");
    result(per, 0.5);
...
int paCallback(const void *inputBuffer, void *outputBuffer,
               ...
               void *userData) {
    std::deque<float> *audioData = reinterpret_cast<std::deque<float> *>(userData);
    const float *readPtr = (const float *)inputBuffer;
    audioData->insert(audioData->end(), readPtr, readPtr + framesPerBuffer);

    if (audioData->size() >= WINDOW_SIZE) {
        analyze_data(*audioData);
        audioData->erase(audioData->begin(), audioData->begin() + framesPerBuffer);
    }
...
int main() {
    ...
    // Record for a certain duration (e.g., 20 seconds)
    Pa_Sleep(20000);
    ...
    //save_audio_data("recorded_audio.wav", audioData);
    Pa_Terminate();
